...
function replaceSeoBlock(html, locale, route) {
    ...
    const alternateLinks = [
        ...localeCodes.map(code => [code, siteUrl + localeRoutes[code]]),
        ['x-default', siteUrl + '/']
...
function translateHomeTemplate(template, localeCode, locales, route, versions, historyCacheVersion, localeBundleCacheVersion) {
    ...
    const localeOptionPattern = new RegExp('(<option value="(?:' + localeCodes.join('|') + ')") selected', 'g');
    html = html.replace(localeOptionPattern, '$1');
...
function buildLandingPages(locales, versions, historyCacheVersion, localeBundleCacheVersion) {
    ...
    localeCodes.forEach(code => {
        writeFile(outputRoutes[code], translateHomeTemplate(template, code, locales, localeRoutes[code], versions, historyCacheVersion, localeBundleCacheVersion));
    });
}
...
function buildSitemap() {
    const urls = [...localeCodes.map(code => localeRoutes[code]), ...subpageSeoDefinitions.map(definition => definition.route), '/privacy/'];
    const xml = [
...
buildSitemap();
buildRobotsTxt();
console.log('Built locales and landing routes: ' + localeCodes.map(code => localeRoutes[code]).join(', ') + ', /kr/');
