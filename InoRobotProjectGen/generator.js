...
const Generator = {
    ...
    InitialProgram(steps, options) {
        ...
        steps.forEach(s => {
            ...
            sb += `        Case ${s.No}11:
            Movl Offset(P${s.No}_Up, PR[B_PR_num]),V[100],Z[1],Tool[${tNum}],Wobj[B_W_num];
            R_Cur_pos = ${s.No}10;
        Case ${s.No}10:
            Movj P${s.No}_Wait,V[100],Z[1],Tool[${tNum}],Wobj[B_W_num];
            R_Cur_pos = ${s.No}01;
`;
...
`;
    ...
    ProcessProgram(s, options, stageIndex) {
        ...
        if (isPeeling) {
            ...
            endAction += `    R_Cur_pos = ${n}20;\n    Movl P${n}_Peel_end,Speed[xwPeel_speed],Z[1],Tool[B_T_num],Wobj[B_W_num];\n`;
        } else if (isCalib) {
            if (tool === "PLC (IO)") {
