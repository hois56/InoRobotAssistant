...
function initApp() {
    ...
    const optDescs = {
        ...
        teachingMode: { title: "Teaching Mode", icon: "pencil", text: `PLC 신호로 로봇 작업 위치를 현장에서 직접 수정할 수 있는 티칭 기능을 추가합니다.<ul class="mt-3 space-y-1 text-left text-xs text-slate-400 list-none"><li>⚙️ <b>각 공정 .pro</b> : P${n}_teaching_mode() 함수 추가 및 work_pos에서 호출</li><li>🏷️ <b>라벨 IN</b> : IN[520] xTeaching_mode, IN[521] xTeaching_save, IN[522] xTeaching_cancel</li><li>🏷️ <b>라벨 OUT</b> : OUT[520] yTeaching_running</li></ul>` },
        waitPos: { title: "Wait Position", icon: "map-pin", text: `공정 시작 전 대기 위치(App→Wait) 이동 기능을 추가합니다.<ul class="mt-3 space-y-1 text-left text-xs text-slate-400 list-none"><li>⚙️ <b>각 공정 .pro</b> : P${n}_wait_pos() 함수 추가 (App→Wait 이동, comp 출력)</li><li>⚙️ <b>각 공정 .pro</b> : work_pos의 Return_move 조건부 호출로 변경</li><li>⚙️ <b>main.pro</b> : 루프 내 Wait pos 섹션 추가 (xP${n}_wait_pos_start 감시)</li><li>🏷️ <b>라벨 IN</b> : IN[528~] xP1_wait_pos_start, xP2_... (프로세스 수만큼)</li><li>🏷️ <b>라벨 OUT</b> : OUT[528~] yP1_wait_pos_comp, yP2_... (프로세스 수만큼)</li></ul>` },
