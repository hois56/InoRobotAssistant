...
- `[출시]` 기존 3D Viewer를 다중 로봇 작업을 구성하고 검증할 수 있는 3D Simulation으로 전면 개편했습니다.
- `[지원]` 제공되는 SCARA 및 6축 로봇 29종의 관절 JOG, Base 좌표 TCP JOG, Tool 좌표계 표시와 수치·3D 핸들 조작을 지원합니다.
- `[지원]` 동일하거나 서로 다른 로봇을 여러 대 배치하고 로봇별 프로그램을 개별 또는 동시에 실행할 수 있습니다.
- `[지원]` MOVJ·MOVL·DELAY·TIME START·TIME OUT, Step Into, 일시정지·재개·정지·반복 및 실시간 사이클 타임 측정을 지원합니다.
- `[지원]` Tool·설비 3D 모델을 TCP 또는 외부 장면에 배치하고 모델 트리, 수치 입력과 3D 핸들로 위치·회전을 조정할 수 있습니다.
- `[지원]` STL, FBX, OBJ, GLB, GLTF, STEP, STP 형식의 3D 모델 불러오기를 지원합니다.
- `[지원]` 모션 프로젝트 자동 저장, JSON 불러오기·내보내기와 Undo·Redo를 지원합니다.
- `[지원]` 모델 트리·JOG·Program Panel의 이동·크기 조절·숨김과 한국어·영어·중국어·베트남어 UI를 지원합니다.
- `[지원]` 3D 시뮬레이션에 가상컨트롤러를 지원합니다. (프로그램 설치 필요)
