...
- `[신규]` 여러 로봇에 독립적인 포인트 프로그램을 구성하고 개별 또는 동시에 실행할 수 있는 멀티 로봇 Program Panel을 추가했습니다.
- `[지원]` MOVJ·MOVL, Step Into, 일시정지·재개·정지·반복 실행을 지원합니다.
- `[추가]` DELAY, TIME START, TIME OUT 명령과 실시간 사이클 타임 측정 기능을 추가했습니다.
- `[추가]` 모션 프로젝트 자동 저장, JSON 불러오기·내보내기 및 실행 결과 Undo·Redo를 지원합니다.
- `[추가]` 로봇 간 충돌 위치와 대상 로봇을 표시하는 시각적 충돌 경고 기능을 추가했습니다.

### Ver 26.07.16.01

- `[출시]` 모든 제공 로봇 모델의 관절 JOG와 Base 좌표 TCP JOG를 지원합니다.
- `[지원]` Tool·설비 3D 모델 불러오기와 모델 트리 기반 배치·수치·3D 핸들 변환을 제공합니다.
- `[지원]` STL, FBX, OBJ, GLB, GLTF, STEP, STP 형식과 실행 취소·다시 실행을 지원합니다.
- `[개편]` 3D Simulation으로 명칭과 화면을 개편하고 이동·분리·숨김 가능한 패널 및 다국어 UI를 적용했습니다.
