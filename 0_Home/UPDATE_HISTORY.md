...
- `[추가]` OLP 워크스페이스에서 로봇 프로젝트 폴더를 불러와 `main.pro` 등 텍스트 파일을 편집하고, 전체 프로젝트를 ZIP 또는 폴더로 저장할 수 있도록 추가했습니다.
- `[추가]` OLP 프로젝트의 포인트 파일을 표 형태로 확인하고 현재 로봇 위치로 포인트를 수정하거나 해당 포인트로 이동하며, 프로그램을 실행·일시정지·정지·단계 실행할 수 있도록 추가했습니다.
- `[추가]` OLP Runtime `R28`과 Virtual Bus·Remote IO 상태, 프로그램 실행 줄·호출 스택·알람 정보를 OLP 화면에서 확인할 수 있도록 추가했습니다.
- `[변경]` 저장된 사용자 뷰를 페이지 안의 고정 뷰 창에서 최대 4개까지 분할해 확인하고, 필요한 경우 별도 창으로 분리할 수 있도록 변경했습니다.
- `[개선]` 충돌 검사 BVH를 모델 로딩 중 미리 준비하고 이동 중 캐시된 삼각형을 재검증하도록 개선하여, 충돌 표시와 반복 검사 동작을 안정화했습니다.
...
- `[변경]` Communication Tester를 `V2.7 → V2.8`로 업데이트하고, Virtual Bus 연결을 OLP 프로젝트 기반의 Master/Slave handshake로 안정화했습니다.
- `[수정]` 이전 연결의 종료 이벤트가 새 Virtual Bus 연결을 끊을 수 있던 경쟁 조건과 handshake 완료 전 연결 상태가 초록색으로 표시되던 문제를 수정했습니다.
- `[개선]` HMI 입력을 최신 IO 상태만 20Hz 단일 송신 루프에서 전송하도록 개선했습니다.
